...
\textit{Copy mutations} introduce an erroneous \textit{substitution} where an instruction is an written into the offspring genome instead of the intended instruction.
In our experiments, substitutions occurred with a per-site probability of 0.0025 \citep{lenski2003evolutionary}.

\textit{Insertion} mutations, by contrast, insert an arbitrary instruction is inserted at random, increasing the length of the offspring's genome by one.
