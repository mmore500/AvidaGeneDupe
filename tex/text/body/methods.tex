...
however, one characteristic in particular worth noting is in how they vary by difficulty in a well-defined manner determined by their intrinsic computational complexity, given in Table \ref{tab:tasks}.
Identical reward was assigned for each task, and kept consistent over the duration of trials.
...
To assess the evolutionary history within a trial, we extracted the trial's final dominant genotype at conclusion and recorded its lineage ancestry.
Processing of lineage data employed Avida's ``analyze mode'' to identify the set of traits performed by each ancestor.
