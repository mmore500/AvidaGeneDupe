...
Coding sites for these traits arising frequently in duplicated regions would suggest that these regions are ``potentiated'' --- that is, posessing latent gene content predisposed to produce a new trait.
Figure \ref{fig:potentiation} compares involvement in coding for new tasks between sites having undergone slip duplication versus those that had not.
...
One possible confounding factor in coding site analysis is evolutionary constraint at genome sites involved in organsims' self-replication loop.
