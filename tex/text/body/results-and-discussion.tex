...
In a first set of experiments, we investigated the impact of slip duplications on \textit{de novo} evolution of phenotypic traits.
We found slip duplications led to significantly more evolved adaptive phenotypic traits than the baseline treatment (two-tailed Mann-Whitney U test, $W = 562.5$, Bonferroni-adjusted $p << 0.0001$; Figure \ref{fig:results_panels:slip_duplication}).
Many replicates evolved larger genomes, some increasing 10-fold over their original 100-instruction starting point.
To test whether this increased genome length was responsible for some benefits seen in the slip-duplication treatment, we performed additional experiments incorporating a ``long-genome'' control treatment initialized using genomes extended to 1,000 sites by padding with instructions that do not perform any computation (no-operations or NOPs).

\input{fig/adaptive-evolution-rate.tex}

Longer genomes alone significantly boosted task acquisition.
This effect was most pronounced for simple tasks (Figure \ref{fig:adaptive-evolution-rate}).
