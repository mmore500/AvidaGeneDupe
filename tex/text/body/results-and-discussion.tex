...
Although a longstanding hypothesis dating as far back as Ohno \citep{ohno1970evolution}, however, results connecting experimental evidence connecting gene duplication, in particular, to complex traits have been less well-established.
...
Interestingly, we did not observe that nop-insert slip-duplication treatment did not produce adaptive benefit.
This likely differs from our long-genome control because the long-genome control was unable to shrink rapidly through deletions, which was forced to retain the deleterious load of mutations associated with longer genome size
Contingent factors may play an important role in preventing genome shrinkage.
We had the same issue with the 100-site genomes, which is why we limited them to not being able to shrink below 100 sites.
...
Indeed, in preliminary experiments we found that deleterious mutational load associated with larger genome size frequently drove genome shinkage, necessitating enforcement of a lower bound on genome size.
...
One possible factour confounding our assay for potentiation is evolutionary constraint at genome sites involved in organsims' self-replication loop.
...
We quantified robustness by counting the number of ``critical'' genome sites, where a single-site knockout underrated replicator viability or one or more adaptive phenotypic traits (i.e., Logic-9 tasks).
\footnote{%
