...
All experiments comprised well-mixed populations with carrying capacity of 3,600 Avida organisms.
To suppy adaptive potential, we adopted the Logic-9 configuration used by Lenski \citet{lenski2003evolutionary} to define a set of nine advantageous phenotypic traits.
Each trait corresponds to a possible logical transform on available binary inputs.
Under this schene, organisms producing correct output values for a task benefit by accelerating their self-copy loop, analogously to a metabolic process.
...
As shown in \ref{fig:slip_mut_variants} the most complex function, EQU, requires five NAND operations while the simplest (NAND and NOT) require only one.
...
Under baseline conditions, these slip allow arbitrary segments of program content to be duplicated or excised.
...
However, as overviewed in Figure \ref{fig:variants}, we also tested a series of mutation operators isolating particular functional effects of gene duplication, in order to tease apart causality in greater detail.
