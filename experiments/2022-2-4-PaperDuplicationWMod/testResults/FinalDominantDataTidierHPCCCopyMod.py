...
def retrieveGenomeLength(organismString):
    analyzeOutputs = organismString.split()
    genomeLength = int(analyzeOutputs[-2])
    return genomeLength
...
#Plucks out task counts and genome lengths
def makeTidy(treatmentArray):
    ...
    with open('FinalDominantGenomeLengths.csv', mode='w') as tidyDat:  
        ...
        for treatment in treatmentArray:
            ...
            for replicateData in treatmentData:
                ...
                for organism in organisms:
                    ID = getOrganismID(organism)
                    updateBorn = getUpdateBorn(organism)
                    genomeLength = retrieveGenomeLength(organism)
                    runName = None
                    for folderName in replicateData.split('/'):
                        if 'run_' not in folderName:
                            continue
                    ...
                    runNum = runParts[-1]
                    params = treatmentParameters[treatmentName]
                    data_writer.writerow([treatmentName,params[0],params[1],params[2],params[3],params[4],params[5],params[6],runNum,ID,updateBorn,genomeLength])
